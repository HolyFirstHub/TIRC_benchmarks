...
void HEMatrixMultiplier::initializeSEAL(size_t poly_modulus_degree) {
    ...
    parms.set_coeff_modulus(seal::CoeffModulus::Create(poly_modulus_degree, {45, 35, 45}));
    ...
    evaluator_ = std::make_unique<seal::Evaluator>(*context_, *encoder_);
...
std::vector<seal::Plaintext> HEMatrixMultiplier::precompute_B_variants(const Matrix& B_plain) {
    validateMatrixDimensions_B(B_plain);

    std::vector<seal::Plaintext> pt_B_variants(M);
    double scale = pow(2.0, 35);

    parallelFor(0, M, [this, &pt_B_variants, &B_plain, scale](int d) {
        auto B_d_packed = packAndShiftMatrixB(B_plain, d);
        encoder_->encode(B_d_packed, scale, pt_B_variants[d]);
    });

    return pt_B_variants;
}

std::vector<seal::Ciphertext> HEMatrixMultiplier::compute_on_server(
    const seal::Ciphertext& ct_A,
    const std::vector<seal::Plaintext>& precomputed_B_variants) {
    ...
    parallelFor(0, M, [this, &diagonals, &ct_A, &precomputed_B_variants](int d) {
        const auto& pt_B_d = precomputed_B_variants[d];

        seal::Ciphertext ct_C_prime_d;
        evaluator_->multiply_plain(ct_A, pt_B_d, ct_C_prime_d);
